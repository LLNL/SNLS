...
struct has_valid_computeRJ <
   CRJ,typename std::enable_if<
       std::is_same<
           decltype(std::declval<CRJ>().computeRJ(std::declval<double* const>(), std::declval<double* const>(),std::declval<const double *>(), std::declval<const int>())),
           bool  
       >::value
       ,
       void
...
// 		     __snls_hdev__ bool computeRJ( double* const r, double* const J, const double* const x, const int nbatch ) ;
...
{
   public:
      static_assert(has_valid_computeRJ<CRJ>::value, "The CRJ implementation in SNLSTrDlDenseG_Batch needs to implement bool computeRJ( double* const r, double* const J, const double* const x, const int nbatch )");
   ...
   public:
   // constructor
   __snls_hdev__ SNLSTrDlDenseG_Batch(CRJ &crj) :
               _crj(crj),
   ...
   public:
      ...
      __snls_hdev__ void   setupSolver(int              maxIter,
         ...
         _status = unConverged ;
         // fix me: modify this to become an array set
         _fevals = 0 ;
      ...
      __snls_hdev__ SNLSStatus_t solve() {
...
#endif
         // fix me: modify this to become a set of temporary arrays
         // we'll probably want to make use of RAJA views to work with these data sets to make
         // our life easier and all
         double residual[_nDim], Jacobian[_nXnDim] ;
         //
         {
            // fix me: we need to modify this to be an array of booleans
            bool rjSuccess;
            this->computeRJ(residual, Jacobian, rjSuccess) ; // at _x
            // fix me this needs to be modified so _status is an array
            // and we don't return early unless all of them failed early on
            if ( !(rjSuccess) ) {
               _status = initEvalFailure ;
               return _status ;
            }
         }
         // fix me: we can make _res and res_0 values be set in a compute kernel
         // that we loop over all the batch data
         // will need to move res_0 value creation outside of this loop though
         _res = this->normvec(residual) ;
         // fix me: this needs to be an array of double
         double res_0 = _res ;
#ifdef __cuda_host_only__
         if (_os) { *_os << "res = " << _res << std::endl ; }
#endif
          // All of these variables need to be extended to arrays and for vector data make use of raja views
         // The initialization of all of this can happen in one single compute kernel        
         ...
         while ( _nIters < _maxIter ) {
            ...
            //
            {
               // These should be lifted out of the loop and just make them an array of values...
               double delx[_nDim] ;
               double pred_resid ;
               bool use_nr = false ;
               //
               ...
               //
               {
                  ...
                  // fix me: the rjSuccess variable needs to be set inside this function call
         ...
         // fix me have this return a status array
         return _status ;
      ...
      __snls_hdev__ inline void computeRJ(double* const r,
                                          double* const J,
         ...
         rJSuccess[0] = this->_crj.computeRJ(r, J, _x, _nbatch);
